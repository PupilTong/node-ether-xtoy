...
static Napi::Value EtherNtoHost(const Napi::CallbackInfo& info) {
    const Napi::Env& env = info.Env();
    if (info.Length() != 1) {
        throw Napi::TypeError::New(env, "Wrong number of arguments");
    }
    ...
    Napi::Buffer<ether_addr> addrBuf = info[0].As<Napi::Buffer<ether_addr> >();
    struct ether_addr* addr = addrBuf.Data();
    char hostNameBuf[1024];
    if(ether_ntohost(hostNameBuf,addr)){
        throwErrnoError(env, errno);
    }
    return Napi::Value::From(env, hostNameBuf);
}

static Napi::Value EtherHosttoN(const Napi::CallbackInfo& info) {
    const Napi::Env& env = info.Env();
    if (info.Length() != 1) {
        throw Napi::TypeError::New(env, "Wrong number of arguments");
    }
	...
	if(ether_ntohost((char*)str_addr.c_str(),&addr)){
        throwErrnoError(env, errno);
	}
	return Napi::Buffer<unsigned char>::Copy(env, reinterpret_cast<const unsigned char*>(&addr), sizeof(ether_addr));
}

static Napi::Object Init(Napi::Env env, Napi::Object exports) {
    exports.Set("ether_ntoa", Napi::Function::New(env, EtherNtoA));
    exports.Set("ether_aton", Napi::Function::New(env, EtherAtoN));
    exports.Set("ether_ntohost", Napi::Function::New(env, EtherNtoHost));
    exports.Set("ether_hostton", Napi::Function::New(env, EtherHosttoN));
    return exports;
}
